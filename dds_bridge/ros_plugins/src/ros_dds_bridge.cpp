...
/**
 * @class ROSDDSBridge
 * @brief A ROS 2 node that periodically publishes `arm_msgs::ArmCommand`
 * messages to a DDS topic and logs the operation to a ROS topic.
 */
class ROSDDSBridge : public rclcpp::Node {
  public:
     ...
     * sets QoS policies, and creates a periodic ROS timer to invoke the DDS
     * callback.
     */
    ROSDDSBridge()
        : Node("ros_dds_bridge"), participant_(0),
        ...
        publisher_ =
            this->create_publisher<std_msgs::msg::String>("dds_status", 10);

        // Set up a timer to publish to DDS every 500ms
        timer_ = this->create_wall_timer(
            3000ms, std::bind(&ROSDDSBridge::dds_callback, this));
    }

  private:
    /**
     * @brief Callback function invoked periodically by the ROS timer.
     * Publishes a sample `ArmCommand` message to the DDS topic and logs the
     * status to a ROS topic.
     */
    void dds_callback() {
        ...
        arm_msgs::ArmCommand command(
            arm_msgs::Header(arm_msgs::Time((total_ns / 1'000'000'000ULL),
                                            (total_ns % 1'000'000'000ULL)),
                             "end_effector_frame"),
            arm_msgs::Pose(arm_msgs::Point(0.0, 0.0, 0.0),
                           arm_msgs::Quaternion(0.0, 0.0, 0.0, 1.0)));

        // Flip x position between 0.0 and 1.0 on every call
        if (flip) {
            command = arm_msgs::ArmCommand(
                arm_msgs::Header(arm_msgs::Time((total_ns / 1'000'000'000ULL),
                                                (total_ns % 1'000'000'000ULL)),
                                 "end_effector_frame"),
                arm_msgs::Pose(arm_msgs::Point(0.5, 0.5, 0.5),
                               arm_msgs::Quaternion(0.0, 0.0, 0.0, 1.0)));
            flip = !flip;
        } else {
            command = arm_msgs::ArmCommand(
                arm_msgs::Header(arm_msgs::Time((total_ns / 1'000'000'000ULL),
                                 ...
                                 "end_effector_frame"),
                arm_msgs::Pose(
                    arm_msgs::Point(0.5, 0.5, 0.5),
                    arm_msgs::Quaternion(0.0, -0.7071068, 0.0, 0.7071068)));
            flip = !flip;
        }
        ...
        try {
            ...
            RCLCPP_INFO(
                this->get_logger(),
                "=== [DDS PUB] Write sample XYZQuat: [%f, %f, %f, %f, %f, %f, "
                "%f]",
                command.pose().position().x(), command.pose().position().y(),
                command.pose().position().z(), command.pose().orientation().x(),
            ...
            RCLCPP_INFO(this->get_logger(),
                        "=== [DDS PUB] Write Sample Successful.");
            success_flag = true;
        ...
        ss << "DDS PUB " << (success_flag ? "SUCCESS" : "FAILURE")
           << "  XYZQuat: [" << command.pose().position().x() << ", "
