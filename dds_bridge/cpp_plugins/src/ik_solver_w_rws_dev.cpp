...
JointConfig solve_ik(RobotModel &robot_model, const pinocchio::SE3 &target_pose,
                     const JointConfig *q_init = nullptr) {
    const int nx = robot_model.getNumVelocityVariables();
    const int n_eq = 0;
    const int n_in = 0;

    Eigen::VectorXd q;
    if (q_init == nullptr)
        q = robot_model.getCurrentJointConfig();
    else
        q = *q_init;
    ...
    for (int iter = 0; iter < 1000; ++iter) {
        ...
        fk_pose = robot_model.computeForwardKinematics(q);
        J = robot_model.computeGeometricJacobian(q); // size 6×nx
        ...
        Eigen::VectorXd dq_lb =
            (robot_model.getJointLimitsLower() - q).cwiseMax(lb);
        Eigen::VectorXd dq_ub =
            (robot_model.getJointLimitsUpper() - q).cwiseMin(ub);
        ...
        while (alpha > alpha_min) {
            ...
            // evaluate new error
            fk_tmp = robot_model.computeForwardKinematics(q_test);
            err_tmp =
    ...
    pinocchio::SE3 final_pose = robot_model.computeForwardKinematics(q);
    std::cout << "Final end-effector pose after IK (SE3):" << std::endl;
    std::cout << final_pose << std::endl;

    // Print the final error after IK
    Eigen::Vector<double, 6> final_error =
        -(pinocchio::log6(target_pose.inverse() * final_pose)).toVector();
    std::cout << "Final error after IK (6D vector):" << std::endl;
    std::cout << final_error.transpose() << std::endl;
    std::cout << "Final error norm: " << final_error.norm() << std::endl;

    robot_model.setCurrentJointConfig(q);
    return q;
}

JointConfig solve_ik(RobotModel &robot_model,
    ...
    target_pose_se3.rotation() =
        Eigen::Quaterniond(target_pose_xyzquat[6], target_pose_xyzquat[3],
            ...
            .toRotationMatrix();
    JointConfig q = solve_ik(robot_model, target_pose_se3, q_init);
    std::cout << "Final end-effector pose after IK (XYZQuat):" << std::endl;
    XYZQuat final_pose_xyzquat = RobotModel::homogeneousToXYZQuat(
        robot_model.computeForwardKinematics(q));
    std::cout << final_pose_xyzquat.transpose() << std::endl;
    return q;
}

JointConfig solve_ik_dls(RobotModel &robot_model,
                         const pinocchio::SE3 &target_pose,
                         const JointConfig *q_init) {
    const int nx = robot_model.getNumVelocityVariables();
    Eigen::VectorXd q =
        (q_init ? *q_init : robot_model.getCurrentJointConfig());

    // Joint limits (user‐provided)
    Eigen::VectorXd q_min = robot_model.getJointLimitsLower();
    Eigen::VectorXd q_max = robot_model.getJointLimitsUpper();
    ...
    for (int iter = 0; iter < 1000; ++iter) {
        // 1) FK & error
        auto fk = robot_model.computeForwardKinematics(q);
        Eigen::Vector<double, 6> err =
        ...
        // 2) Jacobian
        Eigen::MatrixXd J = robot_model.computeGeometricJacobian(q);
...
int main() {
    ...
    solve_ik(robot_model, target_pose_xyzquat, &q_init);
    ...
    auto start_time_solve_ik_no_warmstart_us =
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::high_resolution_clock::now().time_since_epoch())
            .count();
    robot_model.setCurrentJointConfig(RobotModel::NEUTRAL_JOINT_CONFIG);
    solve_ik(robot_model, target_pose_xyzquat);
    ...
    solve_ik_dls(robot_model, target_pose_se3, &q_init);
